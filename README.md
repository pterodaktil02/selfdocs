...
SelfDocs можно запускать без установки Python, nginx и дополнительных компонентов.

Портативная версия предназначена для локальной работы на одном компьютере.

Установка:

1. Скачать архив `SelfDocs-windows-x64.zip`.
2. Распаковать архив в удобный каталог.
3. Запустить `SelfDocs.exe`.

После запуска приложение откроет локальный веб-интерфейс в браузере.

Все рабочие данные хранятся локально.
Интернет для работы приложения не требуется.
